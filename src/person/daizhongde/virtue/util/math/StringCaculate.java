...
public class StringCaculate {
    ...
    private boolean compare(char str) {
        ...
        char last = (char) chs.lastElement().toString().charAt(0);
        switch (str) {
        case '*': {
            // '*/'优先级只比'+-'高
            if (last == '+' || last == '-')
    ...
    public BigDecimal caculate(String st) {
        ...
        return (BigDecimal)numbers.pop();
    }
 
    private void caculate() {
        BigDecimal b = (BigDecimal)numbers.pop();// 第二个运算数
        BigDecimal a = null;// 第一个运算数
        a = (BigDecimal)numbers.pop();
        char ope = chs.pop().toString().charAt(0);
        ...
        case '*':
            ...
            break;
        case '/':
            result = a.divide(a,b.intValue());// 将操作结果放入操作数栈
            numbers.push(result);
            break;
        }
    }
...
//        String a = "(5+-2)*1";
    	String a = "5 * 60 * 60 * 1000";
        StringCaculate caculate = new StringCaculate();
        System.out.println(caculate.parse(a.replaceAll(" ", "")).intValue());
    }
}
