...
public class OracleDBColumnType extends BaseDao{
	//sql_oralce
...
//		return selectColumnNamesAndTypes("SELECT TOP 1000 [n_id] as '哈萨克',[c_number] '骠',[c_password],[c_name],[n_sex],[c_birthday],[c_idcard],[c_mobilenum],[c_mobilenum2],[c_telephoneznum],[c_telephonenum],[c_qq],[c_msn],[c_email],[c_postcode],[c_address],[c_photopath],[n_money],[n_livemoney],[n_cellnum] FROM [db_MoneyManage].[dbo].[tb_authority_user]");
	...
	public static void main(String args[]) throws Exception{
		OracleDBColumnType t = new OracleDBColumnType();
		//cpab gzpost
		String owner = "dshk";
		//TB_PAYER_COMMI_INFO  TB_Area_cd  t_authority_module  t_authority_level
		//hnsd_dshk: TB_EMS_OBRCH  TB_EMS_OPRT  
		//T_EMSDA_COMPLAINT  T_EMSDA_HASTEN  T_EMSDA_HASTEN_R  
		String tableName = "T_EMSDA_COMPLAINT";//for comment query
...
//		print(ColumnType);	print(ColumnTypeName);
...
//		printMap(ColumnLabel,ColumnType);	printMap(ColumnLabel,ColumnTypeName);
...
//		printMap(ColumnLabel,ColumnType,Precision,Scale,ColumnTypeName);
		...
		printSelectSQLNoColAlias(ColumnName, owner+"."+tableName);
		printSelectSQLWithColAlias(ColumnName, owner+"."+tableName);
		printSelectSQLNoTBAlias(ColumnName, owner+"."+tableName);
		printSelectSQLWithTBAliasCopycat(ColumnName, owner+"."+tableName);
		printSelectSQLWithTBAlias(ColumnName, owner+"."+tableName);
...
//				System.out.println("o.toString()"+o.toString()+",ColumnLabel.get(i).toString():"+ColumnLabel.get(i).toString());
	...
	public static void printSelectSQLNoColAlias(Vector ColumnName, String tableName){
		StringBuffer sb = new StringBuffer();
		sb.append("select ");
		...
		System.out.println("\n#### (SQL config) Query.query.NativeSQL No Column Alias ####:\n"+sb);
	}
	
	public static void printSelectSQLWithColAlias(Vector ColumnName, String tableName){
		StringBuffer sb = new StringBuffer();
		sb.append("select ");
		for(int i=0, j=ColumnName.size(); i<j; i++){
			if(i!=j-1){
	...
	public static void printSelectSQLNoTBAlias(Vector ColumnName, String tableName){
		StringBuffer sb = new StringBuffer();
		sb.append("select ");
		for(int i=0, j=ColumnName.size(); i<j; i++){
			if(i!=j-1){
	...
	public static void printSelectSQLWithTBAliasCopycat(Vector ColumnName, String tableName){
		StringBuffer sb = new StringBuffer();
		sb.append("select ");
		for(int i=0, j=ColumnName.size(); i<j; i++){
			if(i!=j-1){
	...
	public static void printSelectSQLWithTBAlias(Vector ColumnName, String tableName){
		StringBuffer sb = new StringBuffer();
		sb.append("select ");
		for(int i=0, j=ColumnName.size(); i<j; i++){
			if(i!=j-1){
